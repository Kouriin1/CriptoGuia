...
const ExchangeRateCard: React.FC = () => {
  ...
  const fetchRate = useCallback(async () => {
    try {
      ...
      // Mantener el último rate si hay error
    } finally {
      setLoading(false);
    }
  }, [rate]);
  ...
  useEffect(() => {
    fetchRate(); // Cargar inmediatamente
    const interval = setInterval(fetchRate, 60000); // Actualizar cada 60 segundos
    return () => clearInterval(interval);
  }, []);

  const isUp = rate !== null && prevRate !== null ? rate >= prevRate : true;
  const displayRate = rate ?? 0;

  // Calcular el cambio porcentual
  const percentChange = rate !== null && prevRate !== null && prevRate !== 0
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
              {loading ? 'Cargando...' : error ? 'Error de conexión' : `Actualizado ${lastUpdate ? lastUpdate.toLocaleTimeString() : ''}`}
            </p>
          </div>
        </div>
      </div>
